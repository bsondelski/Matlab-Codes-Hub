...
m_dot_H=1.5;        %[kg/s]
m_dot_C=1.5;        %[kg/s]
UA_actual=1210;     %conductance [W/K]
substance='air.mix';
mode=3;
...
e=12;                %set error percent to begin while loop
j=1;
err(j)=1;               %set error difference so temp will not change on first iteration

while e>5
    %adjust temperature of hot outlet to be closer to actual
    if err(j)<0 && e>10
        T_H_out(j+1)=T_H_out(j)-1;
    elseif err(j)>0 && e>10
        T_H_out(j+1)=T_H_out(j)+1;
    elseif err(j)<0
        T_H_out(j+1)=T_H_out(j)-0.1;
    elseif err(j)>0
        T_H_out(j+1)=T_H_out(j)+0.1;
    end
    %calculate enthalpies of hot side
    props_H_in=findproperties2(T_H_in,p_H,substance,mode); %hot side inlet properties
    props_H_out=findproperties2(T_H_out(j+1),p_H,substance,mode); %hot side outlet properties
    ...
    T_C_out(j)=findpropertiesT1(h_C_out,p_C,substance,mode); %find outlet temperature for cold side
    ...
    T_C(1)=T_C_out(j);             %cold side outlet temperature
    ...
    for i=2:(N+1)
        ...
        T_H(i)=findpropertiesT1(h_H(i),p_H,substance,mode); %find outlet temperature for hot side sub HEX
        h_C(i)=h_C(i-1)-q_dot/(N*m_dot_C);      %calculate cold side enthalpy at outlet of sub HEX
        T_C(i)=findpropertiesT1(h_C(i),p_C,substance,mode); %find outlet temperature for hot side sub HEX
    end
    ...
    err(j+1)=UA_total-UA_actual;             %difference between UA calculated and given
    e=abs(err(j+1));                         %percent error between UA calc and given
    j=j+1;
end
plot(x,T_C,x,T_H)
%end
