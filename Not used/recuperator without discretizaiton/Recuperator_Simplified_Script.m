...
p=101.325;%               %pressure in system [kPa]
UA=58.4;%%%                %conductance [W/K]
T_C_in=20+273.15;%%%       %cold side inlet temp [K]
T_H_in=60+273.15;%%%       %hot side inlet temp [K]
substance_C='air.mix';%%%%%    %fluid in HEX cold side
substance_H='water';%%%%%%     %fluid in HEX hot side

%create arrays of properties [specific heat[J/kg-K], density [kg/m^3]]
%with inlet temperatures
props_H=findproperties(T_H_in,p,substance_H); %hot side properties at inlet
props_C=findproperties(T_C_in,p,substance_C); %cold side properties at inlet
...
m_dot_C=rho_C*V_dot_C;  %mass flow of cold side fluid [kg/s]
%%%%%%%%%%%%%%%%%%%%%%%%%%%% needs to be changed %%%%%%%%%%%%%%%%%%%%%
m_dot_H=0.03; %rho_H*V_dot_H;  %mass flow of hot side fluid [kg/s]
...
while T_H_error(i-1)>0.0001 || T_C_error(i-1)>0.001
    ...
    props_H_ave=findproperties(T_H_ave,p,substance_H); %hot side properties as average
    props_C_ave=findproperties(T_C_ave,p,substance_C); %cold side properties as average
...
T_H_out=T_H_out(i-1)-273.15
T_C_out=T_C_out(i-1)-273.15
