...
% %Creare
% T_H_in=989;             %K
% T_C_in=367;             %K   there was a mistake 376 K -> dislexic...
% T_H_out=418;            %K
% p_H_in=205;             %kPa
% p_H_out=203;            %kPa
% p_C_in=326;             %kPa
...
inputs = xlsread('C:\Users\sondelski\OneDrive - UW-Madison\nuclear project\Mass curve information\Recuperator Mass Data','Sheet2','B4:M22');
...
for k = 13
    ...
    T_H_in = inputs(k,1);           %K
    T_C_in = inputs(k,2);           %K
    T_H_out = inputs(k,3);          %K
    p_H_in = inputs(k,4)*1000;      %kPa
    p_H_out = inputs(k,5)*1000;     %kPa
    p_C_in = inputs(k,6)*1000;      %kPa
    p_C_out = inputs(k,7)*1000;     %kPa
    m_dot_H = inputs(k,8);          %kg/s
    m_dot_C = inputs(k,9);          %kg/s
    mode = inputs(k,10);
